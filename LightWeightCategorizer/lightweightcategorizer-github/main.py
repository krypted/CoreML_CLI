...
def solution3():
    ...
    category_to_id = OrderedDict(category_id_df.values)
    id_to_category = dict(category_id_df[['category_id', 'Product']].values)
    tfidf = TfidfVectorizer(sublinear_tf=True, min_df=2, norm='l2', encoding='latin-1', ngram_range=(1, 1))#, stop_words='english')
    features = tfidf.fit_transform(df['Consumer complaint narrative']).toarray()
    with open("features.json ","w") as f:
        json.dump(features.tolist(),f,indent=4)
    with open("idf-meta.json ","w") as f:
        json.dump(dict(list(zip(tfidf.get_feature_names(),tfidf.idf_))),f,indent=4)
    ...
    cv_df = pd.DataFrame(index=range(5 * len(_models)))
    entries = []
    ...
    for model in _models:
        ...
        coreml_model = coremltools.converters.sklearn.convert(_models[0], "input_document","category")
        #
        # #set parameters of the model
        coreml_model.short_description = "Categorize an input document using {}".format(model.__class__.__name__)
        coreml_model.input_description["input_document"] = "l2 normed sublinear TFIDF of document to be classified (See idf-meta.json to see vocabulary and idf values to use)"
        coreml_model.output_description["category"] = "Category of the document amongst:- " + ", ".join(list(category_to_id.keys()))
        # #save the model
        coreml_model.save("DocumentCategorizer_{}.mlmodel".format(model.__class__.__name__))
